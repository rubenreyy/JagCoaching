...
export default function ProgressPage({ setCurrentPage, setFeedbackData }) {
  ...
  useEffect(() => {
    const fetchPresentations = async () => {
      try {
        ...
        // Fix the API endpoint URL with the correct path
        const apiUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/videos/presentations/`
        console.log("Fetching presentations from:", apiUrl)
  ...
  // Calculate overall score from feedback data
  ...
  // Generate summary from feedback data
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-3xl font-bold font-mono">{pastPresentations.length}</p>
            </div>
            <div>
          ...
          <Table>
            ...
            <TableBody>
              {filteredPresentations.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 font-mono">
                    {loading ? "Loading presentations..." : "No presentations found. Upload a video to get started!"}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
